...
export default async function updateDisLikeHandler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (_id && name && image) {
      try {
        const result = await db.collection("disLikeFood").insertOne({ _id, name, image, userId });
        return res.status(200).json({ success: true, data: result });
      } catch (error) {
        console.error("싫어요 추가 에러:", error);
    ...
    if (_id) {
      try {
        const result = await db.collection("disLikeFood").deleteOne({ _id, userId });
        return res.status(200).json({ success: true, data: result });
      } catch (error) {
    ...
    try {
      const likeFoods = await db.collection("disLikeFood").find({ userId }).toArray();
      return res.status(200).json({ success: true, data: likeFoods });
    } catch (error) {
      console.error("싫어요 목록 가져오기 에러:", error);
      return res.status(500).json({ error: "서버 에러" });
    }
  } else {
